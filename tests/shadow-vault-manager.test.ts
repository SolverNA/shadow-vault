...
import { IDataAdapter, AdapterStat, ListedFiles } from "../src/adapter-types";
...
// ─────────────────────────────────────────────
// Пользовательские dot-папки (.trash, .git) переживают lock/unlock
// ─────────────────────────────────────────────

describe("ShadowVaultManager — сохранение .trash и dot-папок между сессиями", () => {
  it("полный цикл: файл в shadow/.trash → encrypt-back → удаление shadow (lock) → decrypt (unlock) → файл снова в shadow/.trash", async () => {
    const env = await makeEnv();
    try {
      // Сессия 1: пользователь удалил заметку — Obsidian нативно перенёс её
      // в shadow/.trash (delete-событие уже удалило note.md.enc из оригинала).
      const trashDir = nodePath.join(env.shadowRoot, ".trash");
      fs.mkdirSync(trashDir, { recursive: true });
      fs.writeFileSync(nodePath.join(trashDir, "note.md"), "удалённая заметка");

      // Финальный encrypt-back при shutdown должен захватить .trash
      const r = await env.manager.encryptShadowChangesToOriginal();
      expect(r.failed).toHaveLength(0);
      expect(r.encrypted).toContain(".trash/note.md");

      const encPath = nodePath.join(env.origRoot, ".trash", "note.md.enc");
      expect(fs.existsSync(encPath)).toBe(true);
      expect(env.engine.decryptBuffer(fs.readFileSync(encPath)).toString("utf8"))
        .toBe("удалённая заметка");

      // lock: endSession удаляет shadow целиком
      fs.rmSync(env.shadowRoot, { recursive: true, force: true });
      fs.mkdirSync(env.shadowRoot, { recursive: true });

      // Сессия 2 (unlock): bulk decrypt восстанавливает корзину в shadow
      const dec = await env.manager.decryptAllToShadow();
      expect(dec.failed).toHaveLength(0);
      expect(dec.decrypted).toContain(".trash/note.md");
      expect(fs.readFileSync(nodePath.join(env.shadowRoot, ".trash", "note.md"), "utf8"))
        .toBe("удалённая заметка");
    } finally { env.cleanup(); }
  });

  it("вложенные dot-данные (.git от obsidian-git) тоже шифруются и восстанавливаются", async () => {
    const env = await makeEnv();
    try {
      const gitDir = nodePath.join(env.shadowRoot, ".git", "refs", "heads");
      fs.mkdirSync(gitDir, { recursive: true });
      fs.writeFileSync(nodePath.join(gitDir, "main"), "abc123\n");

      const r = await env.manager.encryptShadowChangesToOriginal();
      expect(r.failed).toHaveLength(0);
      expect(r.encrypted).toContain(".git/refs/heads/main");

      fs.rmSync(env.shadowRoot, { recursive: true, force: true });
      fs.mkdirSync(env.shadowRoot, { recursive: true });

      const dec = await env.manager.decryptAllToShadow();
      expect(dec.decrypted).toContain(".git/refs/heads/main");
      expect(fs.readFileSync(nodePath.join(env.shadowRoot, ".git", "refs", "heads", "main"), "utf8"))
        .toBe("abc123\n");
    } finally { env.cleanup(); }
  });

  it("sync-путь закрытия: hasUnsyncedChangesSync/encryptUnsyncedChangesSync видят файл в .trash", async () => {
    const env = await makeEnv();
    try {
      // Заметка удалена в корзину прямо перед закрытием Obsidian —
      // async encrypt-back не успел, работает только sync-дошифровка.
      const trashDir = nodePath.join(env.shadowRoot, ".trash");
      fs.mkdirSync(trashDir, { recursive: true });
      fs.writeFileSync(nodePath.join(trashDir, "last-minute.md"), "удалена перед закрытием");
      ...
      const enc = nodePath.join(env.origRoot, ".trash", "last-minute.md.enc");
      expect(fs.existsSync(enc)).toBe(true);
      expect(env.engine.decryptBuffer(fs.readFileSync(enc)).toString("utf8"))
        .toBe("удалена перед закрытием");
    } finally { env.cleanup(); }
  });

  it("смена пароля (reEncryptAll) пере-шифровывает и .trash/*.enc", async () => {
    const env = await makeEnv();
    try {
      await writeEncrypted(env, ".trash/old-note.md", "в корзине");

      const newEngine = new CryptoEngine();
      await newEngine.deriveKey("test@test.local", "new-password");
      await env.manager.reEncryptAll(newEngine, () => {});

      const enc = await fsp.readFile(nodePath.join(env.origRoot, ".trash", "old-note.md.enc"));
      expect(newEngine.decryptBuffer(enc).toString("utf8")).toBe("в корзине");
      newEngine.destroy();
    } finally { env.cleanup(); }
  });

  it("exportShadowToOriginal (отключение шифрования) экспортирует .trash и удаляет его .enc", async () => {
    const env = await makeEnv();
    try {
      await writeEncrypted(env, "a.md", "alpha");
      await writeEncrypted(env, ".trash/gone.md", "в корзине");
      await env.manager.decryptAllToShadow();

      const res = await env.manager.exportShadowToOriginal();
      expect(res.failed).toHaveLength(0);
      expect(res.exported.sort()).toEqual([".trash/gone.md", "a.md"]);

      expect(fs.readFileSync(nodePath.join(env.origRoot, ".trash", "gone.md"), "utf8"))
        .toBe("в корзине");
      expect(fs.existsSync(nodePath.join(env.origRoot, ".trash", "gone.md.enc"))).toBe(false);
    } finally { env.cleanup(); }
  });

  it("негатив: служебные артефакты (tmp, .obsidian, .shadow-vault-*, .DS_Store) НЕ шифруются в оригинал", async () => {
    const env = await makeEnv();
    try {
      // Временные файлы атомарной записи
      fs.writeFileSync(nodePath.join(env.shadowRoot, "note.md.shadowtmp"), "tmp");
      fs.writeFileSync(nodePath.join(env.shadowRoot, "a.tmp"), "tmp");
      // Конфиг Obsidian (в тестовом окружении — обычная папка, не symlink)
      fs.mkdirSync(nodePath.join(env.shadowRoot, ".obsidian", "plugins", "shadow-vault"), { recursive: true });
      fs.writeFileSync(
        nodePath.join(env.shadowRoot, ".obsidian", "plugins", "shadow-vault", "session.lock"),
        "{}"
      );
      // Вложенный теневой каталог и мусор Finder
      fs.mkdirSync(nodePath.join(env.shadowRoot, ".shadow-vault-deadbeef"), { recursive: true });
      fs.writeFileSync(nodePath.join(env.shadowRoot, ".shadow-vault-deadbeef", "x.md"), "x");
      fs.writeFileSync(nodePath.join(env.shadowRoot, ".DS_Store"), "junk");

      // Async-путь: ничего из служебного не попадает в encrypt-back
      const r = await env.manager.encryptShadowChangesToOriginal();
      expect(r.encrypted).toHaveLength(0);
      expect(r.failed).toHaveLength(0);

      // Sync-путь: служебное не считается несхороненными изменениями
      expect(env.manager.hasUnsyncedChangesSync()).toBe(false);
      const rs = env.manager.encryptUnsyncedChangesSync();
      expect(rs.encrypted).toBe(0);
      expect(rs.failed).toHaveLength(0);

      // На диске оригинала не появилось .enc для служебных путей
      expect(fs.existsSync(nodePath.join(env.origRoot, "note.md.shadowtmp.enc"))).toBe(false);
      expect(fs.existsSync(nodePath.join(env.origRoot, "a.tmp.enc"))).toBe(false);
      expect(fs.existsSync(nodePath.join(env.origRoot, ".obsidian", "plugins", "shadow-vault", "session.lock.enc"))).toBe(false);
      expect(fs.existsSync(nodePath.join(env.origRoot, ".shadow-vault-deadbeef"))).toBe(false);
      expect(fs.existsSync(nodePath.join(env.origRoot, ".DS_Store.enc"))).toBe(false);
    } finally { env.cleanup(); }
  });

  it("временные файлы ВНУТРИ .trash пропускаются, обычные — шифруются", async () => {
    const env = await makeEnv();
    try {
      const trashDir = nodePath.join(env.shadowRoot, ".trash");
      fs.mkdirSync(trashDir, { recursive: true });
      fs.writeFileSync(nodePath.join(trashDir, "keep.md"), "данные");
      fs.writeFileSync(nodePath.join(trashDir, "junk.md.shadowtmp"), "tmp");

      const r = await env.manager.encryptShadowChangesToOriginal();
      expect(r.encrypted).toEqual([".trash/keep.md"]);
      expect(fs.existsSync(nodePath.join(env.origRoot, ".trash", "junk.md.shadowtmp.enc"))).toBe(false);
    } finally { env.cleanup(); }
  });
});
