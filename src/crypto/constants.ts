...
/**
 * Версия чанкового под-формата с криптографической связкой сегментов (0x04).
 *
 * Проблема 0x03: сегменты аутентифицированы GCM НЕЗАВИСИМО — контейнер,
 * в котором сегменты переставлены/удалены/продублированы, файл обрезан до
 * N первых сегментов или сегмент подставлен из другого файла с тем же ключом,
 * расшифровывается «успешно». 0x04 закрывает это через AAD.
 *
 * Layout (см. crypto/format.ts):
 ...
 *     [segLen u32 LE(4)] [IV(12)] [ciphertext‖tag(16 в конце)]
 *
 * AAD сегмента i = весь 29-байтный заголовок ‖ u32 LE(i).
 * Тем самым GCM-tag каждого сегмента аутентифицирует: его позицию (индекс),
 * общее число сегментов (segCount), blockSize и уникальный fileId файла —
 * перестановка, удаление, дублирование, усечение и кросс-файловая подстановка
 * детектируются как ошибка целостности.
 *
 * ЗАПИСЬ чанкового формата — ТОЛЬКО 0x04. Чтение 0x03 сохранено для
 * совместимости с уже существующими хранилищами.
 */
export const FORMAT_VERSION_CHUNKED2 = 0x04;

/** Длина случайного идентификатора файла в заголовке 0x04 */
export const CHUNK_FILE_ID_LENGTH = 16;
